...
import "./libraries/logic/MaxDepositLogic.sol";
    ...
    PausableUpgradeable
{
    ...
    modifier onlyRole(bytes32 role) {
        require(
            IRoleModule(roleModule).hasRole(role, msg.sender),
            "Not authorized"
        );
        _;
    }
    ...
    // function revokeStrategy(address strategy) public onlyRole(GOVERNANCE_ROLE) {
    ...
    // function shutdownVault() public onlyRole(GOVERNANCE_ROLE) {
    //     depositLimit = 0;
    //     _pause();
    ...
    //     address strategy
    // ) public onlyRole(GOVERNANCE_ROLE) returns (uint256, uint256) {
    ...
    //     address strategy,
    //     uint256 amount
    // ) public nonReentrant onlyRole(GOVERNANCE_ROLE) {

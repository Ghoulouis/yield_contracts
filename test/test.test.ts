...
import { Vault, ERC20Mintable, MockStrategy, ERC20Mintable__factory, Vault__factory, MockStrategy__factory } from "../typechain-types";
import { mint, mintAndDeposit } from "./helper";
describe("Vault", () => {
  let vault: Vault;
  ...
  describe("ERC4626", () => {
    ...
    it("test initialize()", async () => {
      let data = (await vault.vaultData()).addressVault;
      console.log(" data ", data);
      console.log(" vault ", await vault.getAddress());
      expect(data).to.equal(await vault.getAddress());
    });
    ...
    it("test depost()", async () => {
      ...
      let preVaultData = await vault.vaultData();
      ...
      let postVaultData = await vault.vaultData();
      expect(postVaultData.totalIdle).to.equal(preVaultData.totalIdle + amount);
    });
    ...
    it(" test redeem()", async () => {
      let amount = parseUnits("100", 6);
      await mintAndDeposit(vault, usdc, amount, alice);
      let sharesWithdraw = amount / 2n;
      let amountWithdraw = amount / 2n;
      await expect(vault.connect(alice).redeem(sharesWithdraw, alice.address, alice.address))
        .to.be.emit(vault, "Withdrawn")
        .withArgs(alice.address, amountWithdraw, sharesWithdraw, 0);
    });
    ...
    it(" test withdraw()", async () => {
      ...
      let sharesWithdraw = amount / 2n;
      let amountWithdraw = amount / 2n;
      await expect(vault.connect(alice).withdraw(sharesWithdraw, alice.address, alice.address))
        .to.be.emit(vault, "Withdrawn")
        .withArgs(alice.address, amountWithdraw, sharesWithdraw, 0);
    });
  });
